...
var GLOBAL_BASE = 8,
    ...
    STACKTOP = STACK_BASE,
    STACK_MAX = 6096064
    , DYNAMICTOP_PTR = 852976;
    ;


var buffer = new ArrayBuffer(268435456);



var WASM_PAGE_SIZE = 65536;
assert(STACK_BASE % 16 === 0, 'stack must start aligned to 16 bytes, STACK_BASE==' + STACK_BASE);
assert((6096064) % 16 === 0, 'heap must start aligned to 16 bytes, DYNAMIC_BASE==' + 6096064);
...
HEAPU8.set(new Uint8Array(Module['mem']), GLOBAL_BASE);
  ...
  HEAP32[DYNAMICTOP_PTR>>2] = 6096064;
...
// STATICTOP = STATIC_BASE + 853176;



memoryInitializer = "Tiny3D.mem";
...
// {{PRE_LIBRARY}}
  ...
  var _fetch_work_queue=853152;function __emscripten_get_fetch_work_queue() {
...
function nullFunc_vd(x) { abortFnPtrError(x, 'vd'); }
function nullFunc_vdd(x) { abortFnPtrError(x, 'vdd'); }
function nullFunc_vdddd(x) { abortFnPtrError(x, 'vdddd'); }
function nullFunc_vdi(x) { abortFnPtrError(x, 'vdi'); }
...
function nullFunc_vidddd(x) { abortFnPtrError(x, 'vidddd'); }
...
var dynCall_iidiiii = asm["dynCall_iidiiii"];
...
var dynCall_viidi = asm["dynCall_viidi"];
